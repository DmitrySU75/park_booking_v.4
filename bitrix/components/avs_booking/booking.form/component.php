...
if ($request->isPost() && check_bitrix_sessid()) {
    ...
    if (empty($errors)) {
        $timeRange = AVSBookingModule::calculateTimeRange($rentalType, $date, $gazebo['name'], $startHour, $hours);

        if ($timeRange) {
            $restrictions = AVSBookingModule::getDateRestrictions($gazebo['name'], $date);
            $priceModifier = $restrictions['price_modifier'] ?? 1;

            $priceData = \AVS\Booking\TariffManager::calculatePrice($gazebo['name'], $rentalType, $date, $hours, $discountCode);
...
$arResult['RENTAL_TYPES'] = AVSBookingModule::getAvailableRentalTypes($gazebo['name'], $selectedDate);
$arResult['WORK_END_HOUR'] = AVSBookingModule::getWorkEndHour($gazebo['name'], $selectedDate);
