        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success && response.data.is_special) {
                    ...
                    if (response.data.restriction_note) {
                        $('.restriction-note').remove();
                        $('.work-hours-info').after('<div class="restriction-note" style="background:#fff3e0; padding:8px; margin-bottom:15px; border-radius:4px;">⚠️ ' + response.data.restriction_note + '</div>');
                    }
                } else {
