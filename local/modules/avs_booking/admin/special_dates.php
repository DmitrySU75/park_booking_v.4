...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['action'] === 'delete' && check_bitrix_sessid()) {
...
<form method="post">
    ...
    <div class="form-row">
        <label>Беседка:</label>
        <select name="pavilion_name" required>
            <option value="">Выберите беседку</option>
            <?php foreach ($gazebos as $name): ?>
                <option value="<?= htmlspecialcharsbx($name) ?>"><?= htmlspecialcharsbx($name) ?></option>
            <?php endforeach; ?>
        </select>
    </div>
...
<table class="special-table">
    ...
    <tbody>
        <?php foreach ($restrictions as $r): ?>
            <tr>
                <td><?= $r['ID'] ?></td>
                <td><?= htmlspecialcharsbx($r['PAVILION_ID']) ?></td>
