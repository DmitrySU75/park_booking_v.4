-- Таблица заказов
CREATE TABLE IF NOT EXISTS `avs_booking_orders` (
    ...
    `NEW_PAVILION_ID` INT(11) DEFAULT NULL,
    `NEW_PAVILION_NAME` VARCHAR(255) DEFAULT NULL,
    `EXTENDED_END_TIME` DATETIME DEFAULT NULL,
    `PRICE` DECIMAL(10,2) NOT NULL,
    `DEPOSIT_AMOUNT` DECIMAL(10,2) NOT NULL,
    `DISCOUNT_AMOUNT` DECIMAL(10,2) DEFAULT 0,
    `DISCOUNT_CODE` VARCHAR(50) DEFAULT NULL,
    `STATUS` ENUM('pending','paid','confirmed','active','completed','cancelled','deleted') DEFAULT 'pending',
    `PAYMENT_ID` VARCHAR(100) DEFAULT NULL,
    `PAYMENT_STATUS` VARCHAR(50) DEFAULT 'pending',
    `PAID_AMOUNT` DECIMAL(10,2) DEFAULT 0,
    `LIBREBOOKING_RESERVATION_ID` VARCHAR(100) DEFAULT NULL,
    ...
    KEY `idx_pavilion_id` (`PAVILION_ID`),
...
-- Таблица особых дат
CREATE TABLE IF NOT EXISTS `avs_booking_special_dates` (
    `ID` INT(11) NOT NULL AUTO_INCREMENT,
    `PAVILION_ID` INT(11) NOT NULL,
    `DATE` DATE NOT NULL,
    ...
    `UPDATED_AT` DATETIME NOT NULL,
    PRIMARY KEY (`ID`),
    UNIQUE KEY `idx_pavilion_date` (`PAVILION_ID`, `DATE`),
    KEY `idx_date` (`DATE`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Таблица скидок и промокодов
CREATE TABLE IF NOT EXISTS `avs_booking_discounts` (
    `ID` INT(11) NOT NULL AUTO_INCREMENT,
    ...
    `PAVILION_IDS` VARCHAR(255) DEFAULT NULL,
...
-- Таблица логов
CREATE TABLE IF NOT EXISTS `avs_booking_log` (
    `ID` INT(11) NOT NULL AUTO_INCREMENT,
...
-- Таблица уведомлений
CREATE TABLE IF NOT EXISTS `avs_booking_notifications` (
    `ID` INT(11) NOT NULL AUTO_INCREMENT,
...
-- Таблица истории цен
CREATE TABLE IF NOT EXISTS `avs_booking_price_history` (
    `ID` INT(11) NOT NULL AUTO_INCREMENT,
    `PAVILION_ID` INT(11) NOT NULL,
